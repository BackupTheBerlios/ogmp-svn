			...
			if(call && call->status == SIPUA_STATUS_INCOMING)
			{
				...
				if(call->ring_num == 0)
				{
					...
					if(client->background_source)
					{
						...
						client_answer(&client->sipua, call, SIP_STATUS_CODE_TEMPORARILYUNAVAILABLE, NULL);
...
int client_sipua_event(void* lisener, sipua_event_t* e)
{
	...
	switch(e->type)
	{
		...
		case(SIPUA_EVENT_NEW_CALL):
		{
			...
			if(call->status != SIPUA_STATUS_ACCEPT)
			{
				/* Call is declined */
			 ...
			 */
			ret = sipua_negotiate_call(sipua, call, rtpcapset, client->mediatypes, client->default_rtp_ports, client->default_rtcp_ports, client->nmedia,client->control, MAX_CALL_BANDWIDTH);
		...
		case(SIPUA_EVENT_ANSWERED):
		{
			...
			clie_debug (("client_sipua_event: call[%s] answered\n", call->subject));
			clie_debug (("client_sipua_event: SDP\n"));
			clie_debug (("------------------------\n"));
			clie_debug (("%s", sdp_body));
			clie_debug (("------------------------\n"));
			...
			{
				/* Fail to receive the call, miss it */
				sipua_answer(&client->sipua, call, SIP_STATUS_CODE_UNSUPPORTEDMEDIATYPE, NULL);
				break;
			}
#endif
		...
		case(SIPUA_EVENT_ACK):
		{
			...
			bw = sipua_establish_call(sipua, call, "playback", call->rtpcapset,client->format_handlers, client->control, client->pt);
...
char* client_set_call_source(sipua_t* sipua, sipua_call_t* call, media_source_t* source)
{
	...
	sdp = sipua_call_sdp(sipua, call, MAX_CALL_BANDWIDTH, clie->control, clie->mediatypes, clie->default_rtp_ports, clie->default_rtcp_ports, clie->nmedia, source, clie->pt);
	if(sdp)
	{
...
sipua_t* client_new(char *uitype, sipua_uas_t* uas, char* proxy_realm, module_catalog_t* mod_cata, int bandwidth)
{
	...
	client->mediatypes[0] = xstr_clone("audio/speex");
