 ...
 ***************************************************************************/
 ...
 #include <SDL/SDL_thread.h>
   ...
   const int xthread_sdl_log = 1;
 #else          
   const int xthread_sdl_log = 0;
 #endif
 #define xthr_log(fmtargs)  do{if(xthread_sdl_log) printf fmtargs;}while(0)
 ...
 xrtp_thread_t * xthr_new(int (*fn)(void *), void *data, int flag){
    ...
    return (xrtp_thread_t*)SDL_CreateThread(fn, data);
 }
 ...
 int xthr_cond_wait(xthr_cond_t * cond, xthr_lock_t * lock){
    ...
    return SDL_CondWait((SDL_cond*)cond, (SDL_mutex*)lock);
 }
